...
Use the repository commit style from [CONTRIBUTING.md](CONTRIBUTING.md): `<subsystem>(ticdc|dm|engine|all): <what changed>`. Match the scope to the code you touch, for example `worker(dm): improve retry logging` for DM-only changes, `capture(ticdc): add comment for variable declaration` for TiCDC changes, and `sync-diff-inspector: update splitter behavior` for sync-diff-inspector changes. Keep the subject within 70 characters and explain the reason in the body.
