...
alter table t3 add column age int after name;
update t3 set age = 1;
alter table t2 add column info_json json GENERATED ALWAYS AS (`info`) VIRTUAL after name;
update t3 set age = age + 10;
alter table t3 add column info_json json GENERATED ALWAYS AS (`info`) VIRTUAL after name;
alter table t3 add column id_gen int as (uid + 1) after name;
...
alter table t3 add column create_by DATETIME NOT NULL DEFAULT '0000-00-00 00:00:00' after name;
