...
function run() {
	...
	run_sql_tidb_with_retry "SELECT COUNT(*) FROM fk_route_dst.parent_r WHERE parent_id=5;" "COUNT(*): 0"
	run_sql_tidb_with_retry "SELECT parent_id, child_data FROM fk_route_dst.child_r WHERE child_id=50;" "parent_id: 6"
	run_sql_tidb_with_retry "SELECT parent_id, child_data FROM fk_route_dst.child_r WHERE child_id=50;" "child_data: c50_moved"
	run_sql_tidb_with_retry "SELECT COUNT(*) FROM fk_route_dst.child_r c LEFT JOIN fk_route_dst.parent_r p ON c.parent_id=p.parent_id WHERE p.parent_id IS NULL;" "COUNT(*): 0"
}
