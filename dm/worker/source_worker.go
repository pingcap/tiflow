...
func (w *SourceWorker) Start() {
	...
	var err error
	w.sourceDB, err = conn.DefaultDBProvider.Apply(conn.UpstreamDBConfig(&w.cfg.DecryptPassword().From))
	if err != nil {
...
func (w *SourceWorker) updateSourceStatus(ctx context.Context, needLock bool) error {
	...
	if w.sourceDB == nil {
		var err error
		w.sourceDB, err = conn.DefaultDBProvider.Apply(conn.UpstreamDBConfig(&cfg.DecryptPassword().From))
		if err != nil {
			w.sourceDBMu.Unlock()
			return err
		}
