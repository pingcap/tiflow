...
func PutLoadTask(cli *clientv3.Client, task, sourceID, worker string) (int64, error) {
	...
	_, rev, err := etcdutil.DoTxnWithRepeatable(cli, clientv3.OpPut(key, string(data)))
	if err != nil {
		return 0, err
	}
	return rev, nil
...
func DelLoadTask(cli *clientv3.Client, task, sourceID string) (int64, bool, error) {
	...
	resp, rev, err := etcdutil.DoTxnWithRepeatable(cli, clientv3.OpDelete(key))
	if err != nil {
		return 0, false, err
	}
...
func DelLoadTaskByTask(cli *clientv3.Client, task string) (int64, bool, error) {
	...
	resp, rev, err := etcdutil.DoTxnWithRepeatable(cli, clientv3.OpDelete(key, clientv3.WithPrefix()))
	if err != nil {
		return 0, false, err
	}
	return rev, resp.Succeeded, nil
