...
import (
	...
	"github.com/pingcap/tiflow/dm/pkg/mariadbcompat/rules"
	"github.com/pingcap/tiflow/dm/pkg/mariadbcompat/utils"
	"go.uber.org/zap"
)

// charsetRegexPair holds a pre-compiled regex and its replacement string for charset/collation mapping.
type charsetRegexPair struct {
	re          *regexp.Regexp
	replacement string
}
...
type Loader struct {
	...
	logger           *zap.Logger
	charsetRegexes   []charsetRegexPair // pre-compiled charset regexes
	collationRegexes []charsetRegexPair // pre-compiled collation regexes
...
func NewLoaderWithConfig(cfg *config.Config) *Loader {
	...
	// Pre-compile charset mapping regexes
	for source, mapping := range cfg.CharsetMappings {
		re := regexp.MustCompile(`(?i)character\s+set\s+` + regexp.QuoteMeta(source) + `\b`)
		loader.charsetRegexes = append(loader.charsetRegexes, charsetRegexPair{
			re:          re,
			replacement: "CHARACTER SET " + mapping.TargetCharset,
		})
	}

	// Pre-compile collation mapping regexes
	for source, target := range cfg.CollationMappings {
		re := regexp.MustCompile(`(?i)collate\s+` + regexp.QuoteMeta(source) + `\b`)
		loader.collationRegexes = append(loader.collationRegexes, charsetRegexPair{
			re:          re,
			replacement: "COLLATE " + target,
		})
	}
...
// Regexes are pre-compiled once here for efficient reuse.
func (l *Loader) WithCharsetMappings(charsetMap map[string]string, collationMap map[string]string) *Loader {
	l.charsetRegexes = nil
	for source, target := range charsetMap {
		re := regexp.MustCompile(`(?i)character\s+set\s+` + regexp.QuoteMeta(source) + `\b`)
		l.charsetRegexes = append(l.charsetRegexes, charsetRegexPair{
			re:          re,
			replacement: "CHARACTER SET " + target,
		})
	}
	l.collationRegexes = nil
	for source, target := range collationMap {
		re := regexp.MustCompile(`(?i)collate\s+` + regexp.QuoteMeta(source) + `\b`)
		l.collationRegexes = append(l.collationRegexes, charsetRegexPair{
			re:          re,
			replacement: "COLLATE " + target,
		})
	}
...
func (l *Loader) LoadFromFile(filename string) ([]ast.StmtNode, error) {
	l.logger.Debug("Loading SQL file", zap.String("file", filename))
...
func (l *Loader) LoadFromString(sql string) ([]ast.StmtNode, error) {
	...
	l.logger.Debug("Successfully parsed SQL", zap.Int("statements", len(stmts)))
	return stmts, nil
}
...
func (l *Loader) applyCharsetMappings(sql string) string {
	for _, pair := range l.charsetRegexes {
		sql = pair.re.ReplaceAllString(sql, pair.replacement)
	}
	for _, pair := range l.collationRegexes {
		sql = pair.re.ReplaceAllString(sql, pair.replacement)
	}
	return sql
}
