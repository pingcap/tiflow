...
// charsetPrefixRegex matches charset introducer prefixes on empty string defaults
// (e.g. DEFAULT _utf8mb4 '', DEFAULT _latin1 '', DEFAULT _utf8 '').
var charsetPrefixRegex = regexp.MustCompile(`(?i)DEFAULT\s+_[a-z0-9]+\s+''`)
...
func (w *Writer) WriteStatement(stmt ast.StmtNode) (string, error) {
	...
	// Remove charset introducer prefixes on empty string defaults
	result = charsetPrefixRegex.ReplaceAllString(result, "DEFAULT ''")
