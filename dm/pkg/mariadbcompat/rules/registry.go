...
func (r *Registry) Register(rule Rule) {
	r.logger.Debug("Registering rule", zap.String("name", rule.Name()), zap.Int("priority", rule.Priority()))
...
func (r *Registry) registerDefaultRules(cfg *config.Config) {
	r.logger.Debug("Registering default rules")
	...
	for _, rule := range defaultRules {
		if cfg != nil && !cfg.IsRuleEnabled(rule.Name()) {
			r.logger.Debug("Skipping disabled rule", zap.String("name", rule.Name()))
			continue
		}
		r.Register(rule)
	}
