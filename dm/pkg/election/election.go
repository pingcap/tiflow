...
forLoop:
	for i := 0; i < retryCnt; i++ {
		...
		// receive SIGHUP together and tear down concurrently, quorum vanishes
		// mid-Revoke and each non-leader blocks up to sessionTTL (60s) in
		// session.Close(). On normal session expiry the lease is already gone
		// on the server side so Revoke returns quickly anyway.
