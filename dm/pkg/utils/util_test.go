...
func TestIsInterestedDDL(t *testing.T) {
	...
	cases := []struct {
		...
		{"alter database test character set='utf8'", true},
		{"create table test(id int)", true},
		{"alter table tbl add column c1 int", true},
		{"drop table test", true},
		{"truncate table test", true},
		{"rename table test to test1", true},
		{"create index idx_1 on test(id)", true},
		{"drop index idx_1 on test", true},
		...
		// table maintenance
		{"OPTIMIZE TABLE foo", false},
		{"ANALYZE TABLE foo", false},
		{"REPAIR TABLE foo", false},

		// temporary table
		{"DROP /*!40005 TEMPORARY */ TABLE IF EXISTS `h2`", false},
		{"DROP TEMPORARY TABLE IF EXISTS `foo`.`bar` /* generated by server */", false},
		{"DROP TABLE foo.bar", true},
		{"DROP TABLE `TEMPORARY TABLE`", true},
		{"DROP TABLE `TEMPORARY TABLE` /* generated by server */", true},

		// trigger
		{"CREATE DEFINER=`root`@`%` TRIGGER ins_sum BEFORE INSERT ON bar FOR EACH ROW SET @sum = @sum + NEW.id", false},
		{"CREATE TRIGGER ins_sum BEFORE INSERT ON bar FOR EACH ROW SET @sum = @sum + NEW.id", false},
		{"DROP TRIGGER ins_sum", false},
		{"create table `trigger`(id int)", true},

		// procedure
		{"drop procedure if exists prepare_data", false},
		{"CREATE DEFINER=`root`@`%` PROCEDURE `simpleproc`(OUT param1 INT) BEGIN  select count(*) into param1 from shard_0001; END", false},
		{"CREATE PROCEDURE simpleproc(OUT param1 INT) BEGIN  select count(*) into param1 from shard_0001; END", false},
		{"alter procedure prepare_data comment 'i am a comment'", false},
		{"create table `procedure`(id int)", true},
...
END`, false},
		...
		{"ALTER ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `v` AS SELECT qty, price, qty*price AS value FROM t", false},
		{"DROP VIEW v", false},
		{"CREATE TABLE `VIEW`(id int)", true},

		// function
		{"CREATE FUNCTION metaphon RETURNS STRING SONAME 'udf_example.so'", false},
		{"CREATE AGGREGATE FUNCTION avgcost RETURNS REAL SONAME 'udf_example.so'", false},
		{"DROP FUNCTION metaphon", false},
		{"DROP FUNCTION IF EXISTS `rand_string`", false},
		{"ALTER FUNCTION metaphon COMMENT 'hh'", false},
		{"CREATE TABLE `function` (id int)", true},
...
END`, false},
		...
		// tablespace
		{"CREATE TABLESPACE `ts1` ADD DATAFILE 'ts1.ibd' ENGINE=INNODB", false},
		{"ALTER TABLESPACE `ts1` DROP DATAFILE 'ts1.idb' ENGIEN=NDB", false},
		{"DROP TABLESPACE ts1", false},

		// event
		{"CREATE DEFINER=CURRENT_USER EVENT myevent ON SCHEDULE AT CURRENT_TIMESTAMP + INTERVAL 1 HOUR DO UPDATE myschema.mytable SET mycol = mycol + 1;", false},
		{"ALTER DEFINER = CURRENT_USER EVENT myevent ON SCHEDULE EVERY 12 HOUR STARTS CURRENT_TIMESTAMP + INTERVAL 4 HOUR;", false},
		{"DROP EVENT myevent;", false},

		// account management
		{"CREATE USER 't'@'%' IDENTIFIED WITH 'mysql_native_password' AS '*93E34F4B81FEC9E8271655EA87646ED01AF377CC'", false},
		{"ALTER USER 't'@'%' IDENTIFIED WITH 'mysql_native_password' AS '*1114744159A0EF13B12FC371C94877763F9512D0'", false},
		{"rename user t to 1", false},
		{"drop user t1", false},
		{"GRANT ALL PRIVILEGES ON *.* TO 't2'@'%' IDENTIFIED WITH 'mysql_native_password' AS '*12033B78389744F3F39AC4CE4CCFCAD6960D8EA0'", false},
		{"revoke reload on *.* from 't2'@'%'", false},
	}
	for _, ca := range cases {
		require.Equal(t, ca.interested, IsInterestedDDL(ca.sql, p), ca.sql)
	}
}
