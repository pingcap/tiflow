...
func DeleteSourceTables(cli *clientv3.Client, st SourceTables) (int64, error) {
	...
	_, rev, err := etcdutil.DoOpsInOneTxnRepeatableWithRetry(cli, clientv3.OpDelete(key))
	return rev, err
}
...
func GetAllSourceTables(cli *clientv3.Client) (map[string]map[string]SourceTables, int64, error) {
	respTxn, _, err := etcdutil.DoOpsInOneTxnRepeatableWithRetry(cli, clientv3.OpGet(common.ShardDDLOptimismSourceTablesKeyAdapter.Path(), clientv3.WithPrefix()))
	if err != nil {
		return nil, 0, err
	}
