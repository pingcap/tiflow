...
func (t *testDMLSuite) TestGenDMLEvent(c *C) {
	...
	// empty data
	result, err := GenDMLEvents(flavor, serverID, latestPos, latestGTID, replication.WRITE_ROWS_EVENTv2, xid, nil, true, false)
	c.Assert(err, NotNil)
	...
	result, err = GenDMLEvents(flavor, serverID, latestPos, latestGTID, eventType, xid, insertDMLData, true, false)
	c.Assert(err, IsNil)
	...
	result, err = GenDMLEvents(flavor, serverID, latestPos, latestGTID, replication.WRITE_ROWS_EVENTv2, xid, insertDMLData, true, false)
	c.Assert(err, IsNil)
	c.Assert(result, NotNil)
	...
	result, err = GenDMLEvents(flavor, serverID, latestPos, latestGTID, eventType, xid, updateDMLData, true, false)
	c.Assert(err, IsNil)
	c.Assert(result, NotNil)
	...
	result, err = GenDMLEvents(flavor, serverID, latestPos, latestGTID, eventType, xid, deleteDMLData, true, false)
	c.Assert(err, IsNil)
	c.Assert(result, NotNil)
