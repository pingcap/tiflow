...
import (
	...
	"github.com/pingcap/tiflow/pkg/importer"
)
...
func testStructEqual(ctx context.Context, conn *sql.DB, c *C) {
	...
	for _, testCase := range testCases {
		...
		sourceInfo, err := dbutiltest.GetTableInfoBySQL(testCase.createSourceTable, parser.New())
		c.Assert(err, IsNil)

		targetInfo, err := dbutiltest.GetTableInfoBySQL(testCase.createTargetTable, parser.New())
		c.Assert(err, IsNil)
...
func testDataEqual(ctx context.Context, conn *sql.DB, schema string, sourceTables []string, targetTable string, hasEmptyTable bool, c *C) {
	...
	c.Assert(failpoint.Enable("github.com/pingcap/tiflow/pkg/diff/CancelCheckChunkDataEqual", `return(2)`), IsNil)
	defer failpoint.Disable("github.com/pingcap/tiflow/pkg/diff/CancelCheckChunkDataEqual")
