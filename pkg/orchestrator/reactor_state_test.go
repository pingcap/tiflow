...
func (s *stateSuite) TestChangefeedStateUpdate(c *check.C) {
	...
	testCases := []struct {
		...
		{ // common case
			...
			updateValue: []string{
				`{"sink-uri":"blackhole://","opts":{},"create-time":"2020-02-02T00:00:00.000000+00:00","start-ts":421980685886554116,"target-ts":0,"admin-job-type":0,"sort-engine":"memory","sort-dir":"","config":{"case-sensitive":true,"enable-old-value":false,"force-replicate":false,"check-gc-safe-point":true,"filter":{"rules":["*.*"],"ignore-txn-start-ts":null,"ddl-allow-list":null},"mounter":{"worker-num":16},"sink":{"dispatchers":null,"protocol":"default"},"scheduler":{"type":"table-number","polling-time":-1}},"state":"normal","history":null,"error":null,"sync-point-enabled":false,"sync-point-interval":600000000000}`,
				...
				`{"tables":{"45":{"start-ts":421980685886554116}},"operation":null,"admin-job-type":0}`,
				`{"45":{"workload":1}}`,
		...
		{ // test multiple capture
			...
			updateValue: []string{
				...
				`{"tables":{"46":{"start-ts":412341234}},"operation":null,"admin-job-type":0}`,
				`{"46":{"workload":3}}`,
		...
		{ // testing the same key case
			...
			updateValue: []string{
				`{"tables":{"45":{"start-ts":421980685886554116}},"operation":null,"admin-job-type":0}`,
				`{"tables":{"46":{"start-ts":421980685886554116}},"operation":null,"admin-job-type":0}`,
				``,
				`{"tables":{"47":{"start-ts":421980685886554116}},"operation":null,"admin-job-type":0}`,
			},
			expected: ChangefeedReactorState{
				ID: "test1",
				TaskStatuses: map[model.CaptureID]*model.TaskStatus{
