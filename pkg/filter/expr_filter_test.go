...
func TestShouldSkipDMLBasic(t *testing.T) {
	...
	testCases := []testCase{
		{
			ddl: "create table test.student(id int primary key, name char(50), score int, total_score INT GENERATED ALWAYS AS (score + 10) VIRTUAL, gender char(10))",
			cfg: &config.FilterConfig{
				EventFilters: []*config.EventFilterRule{
					{
						Matcher:               []string{"test.student"},
						IgnoreInsertValueExpr: "score >= 20 or gender = 'female'",
						// IgnoreDeleteValueExpr:    "score >= 32 and age < 48",
						// IgnoreUpdateOldValueExpr: "gender = 'male'",
						// IgnoreUpdateNewValueExpr: "score > 28",
					},
				},
			},
			cases: []innerCase{
