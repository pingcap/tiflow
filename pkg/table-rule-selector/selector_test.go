...
func TestSelector(t *testing.T) {
	s := NewTrieSelector()
	suite.expectedSchemaRules, suite.expectedTableRules = suite.testGenerateExpectedRules()

	suite.testInsert(t, s)
	suite.testMatch(t, s)
	suite.testAppend(t, s)
	suite.testReplace(t, s)
	suite.testRemove(t, s)
}
...
func (suite *testSelectorSuite) testInsert(t *testing.T, s Selector) {
	var err error
	for schema, rules := range suite.expectedSchemaRules {
	...
	for schema, tables := range suite.expectedTableRules {
	...
	c.Assert(schemas, DeepEquals, suite.expectedSchemaRules)
	c.Assert(tables, DeepEquals, suite.expectedTableRules)
}

func (suite *testSelectorSuite) testRemove(c *C, s Selector) {
	for i := 0; i < len(suite.removeCases); i += 2 {
		schema, table := suite.removeCases[i], suite.removeCases[i+1]
		...
		if len(table) == 0 {
			delete(suite.expectedSchemaRules, schema)
		} else {
			rules, ok := suite.expectedTableRules[schema]
			c.Assert(ok, IsTrue)
			delete(rules, table)
		}
	}
...
func (suite *testSelectorSuite) testAppend(c *C, s Selector) {
	var (
		err          error
		appendedRule = &dummyRule{description: "append"}
	)
	for schema := range suite.expectedSchemaRules {
		suite.expectedSchemaRules[schema] = append(suite.expectedSchemaRules[schema], appendedRule)
...
func (suite *testSelectorSuite) testReplace(c *C, s Selector) {
	var (
		err          error
		replacedRule = &dummyRule{"replace"}
	)
	for schema := range suite.expectedSchemaRules {
		suite.expectedSchemaRules[schema] = []interface{}{replacedRule}
...
func (suite *testSelectorSuite) testMatch(c *C, s Selector) {
	cache := make(map[string]RuleSet)
	for _, mc := range suite.matchCase {
...
func (suite *testSelectorSuite) testGenerateExpectedRules() (map[string][]interface{}, map[string]map[string][]interface{}) {
	schemaRules := make(map[string][]interface{})
	tableRules := make(map[string]map[string][]interface{})
	for schema, tables := range suite.tables {
