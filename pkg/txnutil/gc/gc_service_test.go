...
func TestCheckSafetyOfStartTs(t *testing.T) {
	...
	err := EnsureChangefeedStartTsSafety(ctx, pdCli, "changefeed1", TTL, 50)
	require.Equal(t, err.Error(), "[CDC:ErrStartTsBeforeGC]fail to create changefeed because start-ts 50 is earlier than GC safepoint at 60")
	...
	err = EnsureChangefeedStartTsSafety(ctx, pdCli, "changefeed2", TTL, 65)
	require.Nil(t, err)
	...
	require.Equal(t, err.Error(), "[CDC:ErrReachMaxTry]reach maximum try: 9: not pd leader")
