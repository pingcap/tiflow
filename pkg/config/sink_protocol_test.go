...
func TestParseSinkProtocolFromString(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			protocol:    "random",
			expectedErr: ".*unknown 'random' message protocol for sink.*",
		},
		{
			protocol:             "default",
...
func TestShouldSplitUpdate(t *testing.T) {
	t.Parallel()
	...
	testCases := []struct {
		...
		{
			protocolEnum: ProtocolCsv,
			expect:       true,
		},
	}

	for _, tc := range testCases {
		require.Equal(t, tc.expect, tc.protocolEnum.ShouldSplitUpdate())
	}
}
