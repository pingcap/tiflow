...
func (b *bootstrapWorker) generateEvents(
	...
	events := make([]*future, 0, int(totalPartition))
...
func newTableStatus(key TopicPartitionKey, row *model.RowChangedEvent) *tableStatus {
	...
	res.lastMsgReceivedTime.Store(time.Unix(0, 0))
...
func (t *tableStatus) isInactive(maxInactiveDuration time.Duration) bool {
