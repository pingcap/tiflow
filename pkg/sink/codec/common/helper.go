...
func MustSnapshotQuery(
	...
	for rows.Next() {
		...
		for i, b := range holder.rawValues {
			// A nil byte slice from Scan signals SQL NULL. Preserve that as an
			// untyped nil so downstream `value == nil` checks behave the same
			// way they did before the *[]byte scan was introduced.
			if b == nil {
				holder.Values[i] = nil
			} else {
				holder.Values[i] = b
