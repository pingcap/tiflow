...
func RegisterRoutes(
	...
	//ballast := newBallast(1024 * 1024 * 1024 * 2)
	////ballast := newBallast(s.cfg.MaxBallastObjectSize)
	//if err := ballast.SetSize(s.cfg.BallastObjectSize); err != nil {
	//	log.Warn("set initial ballast object size failed", zap.Error(err))
	//}

	//router.Group("/debug").GET("/ballast", api.GetBallast)
	//router.Group("/debug").POST("/ballast", api.SetBallast)

	// allocate ballast
	_ = make([]byte, 1024*1024*1024*2)
}
...
//		if err != nil {
//			log.Warn("failed to get the total amount of RAM on this system", zap.Error(err))
//		} else {
