...
func TestNewClient(t *testing.T) {
	...
	cli := NewCDCClient(context.Background(), pdClient, nil, grpcPool, regionCache, pdtime.NewClock4Test(), model.DefaultNamespaceChangeFeedID(""))
...
func TestConnectOfflineTiKV(t *testing.T) {
	...
	regionCache := tikv.NewRegionCache(pdClient)
	defer regionCache.Close()
	cdcClient := NewCDCClient(context.Background(), pdClient, kvStorage, grpcPool, regionCache, pdtime.NewClock4Test(), model.DefaultNamespaceChangeFeedID(""))
...
func TestStreamSendWithError(t *testing.T) {
	...
	lockerResolver := txnutil.NewLockerResolver(kvStorage, model.DefaultNamespaceChangeFeedID("changefeed-test"), util.RoleTester)
	isPullInit := &mockPullerInit{}
	grpcPool := NewGrpcPoolImpl(ctx, &security.Credential{})
	defer grpcPool.Close()
	regionCache := tikv.NewRegionCache(pdClient)
