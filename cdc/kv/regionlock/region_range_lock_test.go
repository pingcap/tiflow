...
func mustSuccess(t *testing.T, res LockRangeResult, expectedWatermark uint64) {
	require.Equal(t, LockRangeStatusSuccess, res.Status)
	require.Equal(t, expectedWatermark, res.Watermark)
}
...
func mustLockRangeSuccess(
	...
	regionID, version, expectedWatermark uint64,
) {
	res := l.LockRange(ctx, []byte(startKey), []byte(endKey), regionID, version)
	mustSuccess(t, res, expectedWatermark)
}
...
func unlockRange(l *RegionRangeLock, startKey, endKey string, regionID, version uint64, watermark uint64) {
	l.UnlockRange([]byte(startKey), []byte(endKey), regionID, version, watermark)
}
...
func TestRegionRangeLockLockingRegionID(t *testing.T) {
	...
	// the result's watermark should be 20 from of range ["g", "h"), instead of 11 from min(11, 20).
...
func TestRegionRangeLockCollect(t *testing.T) {
	...
	l.LockRange(ctx, []byte("a"), []byte("m"), 1, 1).LockedRange.Watermark.Add(1)
	attrs = l.CollectLockedRangeAttrs(nil)
	require.Equal(t, 1, len(attrs.Holes))
	require.Equal(t, uint64(101), attrs.SlowestRegion.Watermark)
	require.Equal(t, uint64(101), attrs.FastestRegion.Watermark)

	l.LockRange(ctx, []byte("m"), []byte("z"), 2, 1).LockedRange.Watermark.Add(2)
	attrs = l.CollectLockedRangeAttrs(nil)
	require.Equal(t, 0, len(attrs.Holes))
	require.Equal(t, uint64(101), attrs.SlowestRegion.Watermark)
	require.Equal(t, uint64(102), attrs.FastestRegion.Watermark)

	l.UnlockRange([]byte("a"), []byte("m"), 1, 1)
	attrs = l.CollectLockedRangeAttrs(nil)
	require.Equal(t, 1, len(attrs.Holes))
	require.Equal(t, uint64(102), attrs.SlowestRegion.Watermark)
	require.Equal(t, uint64(102), attrs.FastestRegion.Watermark)
...
func TestCalculateMinWatermark(t *testing.T) {
	l := NewRegionRangeLock(1, []byte("a"), []byte("z"), 100, "")

	res := l.LockRange(context.Background(), []byte("m"), []byte("x"), 1, 1)
	res.LockedRange.Watermark.Store(101)
	require.Equal(t, LockRangeStatusSuccess, res.Status)
	require.Equal(t, uint64(100), l.CalculateMinWatermark())

	res = l.LockRange(context.Background(), []byte("a"), []byte("m"), 2, 1)
	require.Equal(t, LockRangeStatusSuccess, res.Status)
	res.LockedRange.Watermark.Store(102)
	res = l.LockRange(context.Background(), []byte("x"), []byte("z"), 3, 1)
	require.Equal(t, LockRangeStatusSuccess, res.Status)
	res.LockedRange.Watermark.Store(103)
	require.Equal(t, uint64(101), l.CalculateMinWatermark())
}
...
func BenchmarkOneMillionRegions(b *testing.B) {
	...
	for i := 1; i <= 1000*1000; i++ {
		...
		lockRes.LockedRange.Watermark.Store(uint64(100 + i))
	}

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		l.CalculateMinWatermark()
	}
}
