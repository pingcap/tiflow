...
func TestRouter(t *testing.T) {
	...
	for i := 0; i < limit; i++ {
		r.AddRegion(singleRegionInfo{checkpointTs: uint64(i), rpcCtx: &tikv.RPCContext{Addr: store}})
	}
	...
	for i := 0; i < limit; i++ {
		select {
		case sri := <-r.Chan():
			require.Equal(t, uint64(i), sri.checkpointTs)
			r.Acquire(store)
...
func TestRouterWithMultiStores(t *testing.T) {
	...
	for _, store := range stores {
		for j := 0; j < limit*2; j++ {
			r.AddRegion(singleRegionInfo{checkpointTs: uint64(j), rpcCtx: &tikv.RPCContext{Addr: store}})
		}
	}
	received := uint64(0)
	wg, ctx := errgroup.WithContext(ctx)
