...
func testMounterDisableOldValue(t *testing.T, tc struct {
	...
	tableInfo := tk.DDL2TableInfo(tc.createTableDDL)
	...
	var count int
	events := tk.GetAllEventsByTable("test", tc.tableName)
	for _, event := range events {
		if event == nil {
			continue
		}
		count++
		require.Equal(t, event.Table.Table, tc.tableName)
		require.Equal(t, event.Table.Schema, "test")
		t.Log("ApproximateBytes", tc.tableName, count-1, event.ApproximateBytes())
		if len(event.Columns) != 0 {
			checkSQL, params := prepareCheckSQL(t, tc.tableName, event.Columns)
			result := tk.MustQuery(checkSQL, params...)
			result.Check([][]interface{}{{"1"}})
		}
		if len(event.PreColumns) != 0 {
			checkSQL, params := prepareCheckSQL(t, tc.tableName, event.PreColumns)
			result := tk.MustQuery(checkSQL, params...)
			result.Check([][]interface{}{{"1"}})
		}
	}
	require.Equal(t, count, len(tc.values))
	//
	//for _, params := range tc.values {
	//	deleteSQL := prepareDeleteSQL(t, tableInfo, len(params))
	//	tk.MustExec(deleteSQL, params...)
	//}
...
//func prepareDeleteSQL(t *testing.T, tableInfo *model.TableInfo, columnLens int) string {
//	quotedTable := tableInfo.TableName.QuoteString()
//	var sb strings.Builder
//	_, err := sb.WriteString("DELETE FROM " + quotedTable + " WHERE ")
...
-- gbk dmls
  ...
  image tinyblob
	...
	tableInfo := tk.DDL2TableInfo(createTableSQL)
...
func TestDecodeRowEnableChecksum(t *testing.T) {
	...
	tableInfo := tk.DDL2TableInfo(createTableDDL)
	...
	tableInfo = tk.DDL2TableInfo("alter table t change column a a varchar(10)")
	...
	_ = tk.DDL2TableInfo("drop table t")
}
