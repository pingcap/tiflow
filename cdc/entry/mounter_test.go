...
func (cs *mountTxnsSuite) TestInsertPkNotHandle(c *check.C) {
	...
	t, err := mounter.Mount(txn.RawTxn{
		Ts:      rawKV[0].Ts,
		Entries: rawKV,
	})
	c.Assert(err, check.IsNil)
	cs.assertTableTxnEquals(c, t, &txn.Txn{
		Ts: rawKV[0].Ts,
		DMLs: []*txn.DML{
			{
				Database: "testDB",
...
func (cs *mountTxnsSuite) TestDDL(c *check.C) {
	...
	c.Assert(err, check.IsNil)
	c.Assert(t, check.DeepEquals, &txn.Txn{
		DDL: &txn.DDL{
			Database: "testDB",
			Table:    "test1",
			Job:      &model.Job{},
