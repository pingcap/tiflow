...
func TestConvertNilRowChangedEvents(t *testing.T) {
	...
	result, size, err := convertRowChangedEvents(changefeedID, span, enableOldVlaue, sink.MySQLScheme, events...)
...
func TestConvertEmptyRowChangedEvents(t *testing.T) {
	...
	result, size, err := convertRowChangedEvents(changefeedID, span, enableOldValue, sink.MySQLSSLScheme, events...)
	require.NoError(t, err)
	require.Equal(t, 0, len(result))
	require.Equal(t, uint64(0), size)
}
