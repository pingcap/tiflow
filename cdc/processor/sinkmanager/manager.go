...
type Manager interface {
	...
	// It is used to advance the progress if RedoLog is not enabled.
	UpdateBarrierTs(ts model.Ts)
	// UpdateTableResolvedTs updates the resolved ts of a table.
	// It used to advance the progress if RedoLog is enabled.
	UpdateTableResolvedTs(tableID model.TableID, ts model.Ts)
	// AddTable adds a table(TableSink) to the sink manager.
	// Sink manager will create a new table sink and start it.
	AddTable(tableID model.TableID, startTs model.Ts)
	...
	GetTableStats(tableID model.TableID) pipeline.Stats
