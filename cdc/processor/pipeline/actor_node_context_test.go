...
func TestContext(t *testing.T) {
	t.Parallel()
	ctx := NewContext(sdtContext.TODO(), "a.test", nil, 1, &context.ChangefeedVars{ID: "zzz"}, &context.GlobalVars{}, throwDoNothing)
...
func TestTryGetProcessedMessageFromChan(t *testing.T) {
	t.Parallel()
	ctx := NewContext(sdtContext.TODO(), "a.test", nil, 1, nil, nil, throwDoNothing)
...
func TestThrow(t *testing.T) {
	...
	a := 0
	tf := func(err error) { a++ }
	actorContext := NewContext(sdtContext.TODO(), "a.test", nil, 1, nil, nil, tf)
...
func TestSendToNextNodeNoTickMessage(t *testing.T) {
	...
	actorContext := NewContext(ctx, "a.test", sys.Router(), actorID, &context.ChangefeedVars{ID: "abc"}, &context.GlobalVars{}, throwDoNothing)
