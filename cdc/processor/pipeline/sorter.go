	...
	for i < len(message) {
		...
		case msg, ok := <-output:
			...
			if msg.RawKV.OpType == model.OpTypeResolved {
				...
				message[i] = pmessage.PolymorphicEventMessage(msg)
				i++
			...
			if msg.RawKV.OpType != model.OpTypeResolved {
				...
				commitTs := msg.CRTs
				// this is the first one message with commitTs larger than the global resolvedTs,
				// also return it since it already out of the sorter, to prevent potential data loss.
				// and return immediately.
				if commitTs > globalResolvedTs {
					message[i] = pmessage.PolymorphicEventMessage(msg)
					i++
					return nil
				}
