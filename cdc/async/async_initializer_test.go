...
func TestTryInitialize(t *testing.T) {
	initializer := NewInitializer(func(ctx cdcContext.Context) error {
		return nil
	})
	pool := &cdcContext.NonAsyncPool{}
	initialized, err := initializer.TryInitialize(cdcContext.NewContext4Test(context.Background(), false), pool)
	require.Nil(t, err)
	...
	initialized, err = initializer.TryInitialize(cdcContext.NewContext4Test(context.Background(), false), &fakePool{submitErr: errors.New("submit error")})
	require.NotNil(t, err)
	require.False(t, initialized)
	require.False(t, initializer.initialized.Load())
	require.True(t, initializer.initializing.Load())
...
func TestTerminate(t *testing.T) {
	...
	fpool := &fakePool{}
	initialized, err = initializer.TryInitialize(cdcContext.NewContext4Test(context.Background(), false), fpool)
	require.Nil(t, err)
	require.False(t, initialized)
	require.True(t, initializer.initializing.Load())
