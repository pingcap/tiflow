...
func TestManualMoveTable(t *testing.T) {
	...
	// Test moving to a non-existent capture
	dispatcher.MoveTable(2, "capture-200")
	communicator.Reset()
	checkpointTs, resolvedTs, err = dispatcher.Tick(ctx, 1300, []model.TableID{1, 2, 3}, defaultMockCaptureInfos)
	require.NoError(t, err)
	require.Equal(t, model.Ts(1300), checkpointTs)
...
func TestManualRebalance(t *testing.T) {
	...
	communicator.On("DispatchTable", mock.Anything, "cf-1", mock.Anything, "capture-1", true).
		Return(true, nil)
	communicator.On("DispatchTable", mock.Anything, "cf-1", mock.Anything, "capture-2", true).
		Return(true, nil)
	...
	for captureID, tables := range communicator.removeTableRecords {
		for _, tableID := range tables {
			dispatcher.OnAgentFinishedTableOperation(captureID, tableID)
			communicator.On("DispatchTable", mock.Anything, "cf-1", tableID, "capture-3", false).
				Return(true, nil)
		}
	}
	checkpointTs, resolvedTs, err = dispatcher.Tick(ctx, 1300, []model.TableID{1, 2, 3, 4, 5, 6}, mockCaptureInfos)
