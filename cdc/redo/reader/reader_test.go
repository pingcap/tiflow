...
func TestLogReader_ResetReader(t *testing.T) {
	...
	fileName := fmt.Sprintf("%s_%s_%d_%s_%d%s", "cp", "test-cf100", time.Now().Unix(), redo.DefaultDDLLogFileName, 100, redo.LogEXT)
	path := filepath.Join(dir, fileName)
	f, err := os.Create(path)
	assert.Nil(t, err)
	fileName = fmt.Sprintf("%s_%s_%d_%s_%d%s", "cp", "test-cf10", time.Now().Unix(), redo.DefaultRowLogFileName, 10, redo.LogEXT)
	path = filepath.Join(dir, fileName)
	...
	for _, tt := range tests {
		...
		r := &LogReader{
			...
			meta:      &redo.LogMeta{CheckPointTs: tt.args.checkPointTs, ResolvedTs: tt.args.resolvedTs},
		}
...
func TestLogReader_ReadMeta(t *testing.T) {
	...
	assert.Nil(t, err)
	defer os.RemoveAll(dir)
	fileName := fmt.Sprintf("%s_%s_%d_%s%s", "cp", "test-changefeed", time.Now().Unix(), redo.DefaultMetaFileName, redo.MetaEXT)
	path := filepath.Join(dir, fileName)
	f, err := os.Create(path)
	assert.Nil(t, err)
	meta := &redo.LogMeta{
