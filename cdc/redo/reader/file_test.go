...
func TestReaderRead(t *testing.T) {
	...
	fileName := fmt.Sprintf("%s_%s_%s_%d_%s_%d%s", cfg.CaptureID, cfg.ChangeFeedID.Namespace, cfg.ChangeFeedID.ID, cfg.CreateTime.Unix(), cfg.FileType, 11, common.LogEXT)
...
func TestReaderOpenSelectedFiles(t *testing.T) {
	...
	fileName := fmt.Sprintf("%s_%s_%s_%d_%s_%d%s", "cp", "default", "test-cf", time.Now().Unix(), common.DefaultDDLLogFileType, 11, common.LogEXT+common.TmpEXT)
	...
	fileName = fmt.Sprintf("%s_%s_%s_%d_%s_%d%s", "cp", "default", "test-cf11", time.Now().Unix(), common.DefaultDDLLogFileType, 10, common.LogEXT)
	path = filepath.Join(dir, fileName)
	_, err = os.Create(path)
	require.Nil(t, err)

	// SortLogEXT, wil open
	fileName = fmt.Sprintf("%s_%s_%s_%d_%s_%d%s", "cp", "default", "test-cf111", time.Now().Unix(), common.DefaultDDLLogFileType, 10, common.LogEXT) + common.SortLogEXT
	path = filepath.Join(dir, fileName)
	...
	fileName = fmt.Sprintf("%s_%s_%s_%d_%s_%d%s", "cp", "default", "test-cf", time.Now().Unix(), common.DefaultDDLLogFileType, 11, common.LogEXT+"test")
