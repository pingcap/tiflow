...
func TestPreCheck(t *testing.T) {
	...
	require.Equal(t, cf.runningStatus, changeFeedClosed)
}
...
func TestInitialize(t *testing.T) {
	...
	require.Equal(t, cf.runningStatus, changeFeedRunning)
}
...
func TestChangefeedHandleError(t *testing.T) {
	...
	require.Equal(t, cf.runningStatus, check.Equals, changeFeedClosing)

	cf.Tick(ctx, state, captures)
	require.Equal(t, cf.runningStatus, changeFeedClosed)
}

func TestHandleAdminJob(t *testing.T) {
	ctx := cdcContext.NewBackendContext4Test(true)
	cf, state, captures, tester := createChangefeed4Test(ctx, t)
	defer cf.Close(ctx)
	// pre check
	...
	cf.Tick(ctx, state, captures)
	tester.MustApplyPatches()
	require.Equal(t, cf.runningStatus, changeFeedClosing)
...
func TestRemoveChangefeed(t *testing.T) {
	...
	testChangefeedReleaseResource(t, ctx, cancel, dir, changeFeedRunning)
}
...
func TestRemovePausedChangefeed(t *testing.T) {
	...
	testChangefeedReleaseResource(t, ctx, cancel, dir, changeFeedClosed)
}
...
func testChangefeedReleaseResource(
	...
	cf.Tick(ctx, state, captures)
	tester.MustApplyPatches()
	require.Equal(t, cf.runningStatus, expectedRunningStatus)
