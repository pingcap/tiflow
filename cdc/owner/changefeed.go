...
func (c *changefeed) tick(ctx cdcContext.Context, state *orchestrator.ChangefeedReactorState, captures map[model.CaptureID]*model.CaptureInfo) error {
	...
	if barrierTs < checkpointTs {
		...
		// So we return here.
		log.Info("barrierTs < checkpointTs, premature to schedule tables or update status", zap.String("changefeed", c.id),
...
func (c *changefeed) asyncExecDDL(ctx cdcContext.Context, job *timodel.Job) (done bool, err error) {
	if job.BinlogInfo == nil {
		log.Warn("ignore the invalid DDL job", zap.String("changefeed", c.id), zap.Reflect("job", job))
		return true, nil
	}
	cyclicConfig := c.state.Info.Config.Cyclic
	if cyclicConfig.IsEnabled() && !cyclicConfig.SyncDDL {
		log.Info("ignore the DDL job because cyclic config is enabled and syncDDL is false", zap.String("changefeed", c.id), zap.Reflect("job", job))
		return true, nil
	}
	if c.ddlEventCache == nil || c.ddlEventCache.CommitTs != job.BinlogInfo.FinishedTS {
		ddlEvent, err := c.schema.BuildDDLEvent(job)
		if err != nil {
			log.Error("build DDL event fail", zap.String("changefeed", c.id), zap.Reflect("job", job), zap.Error(err))
		...
		if err != nil {
			log.Error("add special comment fail", zap.String("changefeed", c.id), zap.String("Query", ddlEvent.Query), zap.Error(err))
			return false, errors.Trace(err)
		}
	...
	if job.BinlogInfo.TableInfo != nil && c.schema.IsIneligibleTableID(job.BinlogInfo.TableInfo.ID) {
		log.Warn("ignore the DDL job of ineligible table", zap.String("changefeed", c.id), zap.Reflect("job", job))
		return true, nil
	}
