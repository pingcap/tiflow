...
func TestPrepareDML(t *testing.T) {
	...
	for _, tc := range testCases {
		txns := make([]*model.SingleTableTxn, 0)
		txn := &model.SingleTableTxn{
			Rows: tc.input,
		}
		txns = append(txns, txn)
		dmls := ms.prepareDMLs(txns, 0, 0)
...
func TestExecDMLRollbackErrDatabaseNotExists(t *testing.T) {
	...
	err = sink.execDMLs(ctx, txns, 1 /* replicaID */, 1 /* bucket */)
...
func TestMysqlSinkNotRetryErrDupEntry(t *testing.T) {
	...
	f, err := filter.NewFilter(rc)
	require.Nil(t, err)
	sink, err := NewMySQLSink(
		ctx, model.DefaultChangeFeedID(changefeed), sinkURI, f, rc, map[string]string{})
	require.Nil(t, err)
	...
	err = sink.execDMLs(ctx, txns, 0, 1 /* bucket */)
...
func TestGroupRowsByType(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		ms.params.maxTxnRow = tc.maxTxnRow
		inserts, updates, deletes := ms.groupRowsByType(tc.input, tableInfo, false)
