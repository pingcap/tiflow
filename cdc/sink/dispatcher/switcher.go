...
func (s *EventSwitcher) DispatchRowChangedEvent(row *model.RowChangedEvent) (string, int32) {
	partitionDispatcher, topicDispatcher := s.matchDispatcher(row)
...
func (s *EventSwitcher) matchDispatcher(row *model.RowChangedEvent) (partition.Dispatcher, topic.Dispatcher) {
	for _, rule := range s.rules {
		if !rule.MatchTable(row.Table.Schema, row.Table.Table) {
			continue
		}
		return rule.partitionDispatcher, rule.topicDispatcher
	}
...
// NewEventSwitcher creates a new EventSwitcher
func NewEventSwitcher(cfg *config.ReplicaConfig, partitionNum int32, defaultTopic string) (*EventSwitcher, error) {
