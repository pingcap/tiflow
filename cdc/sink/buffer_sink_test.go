...
func TestFlushTable(t *testing.T) {
	...
	b := newBufferSink(ctx, newBlackHoleSink(ctx, make(map[string]string)), make(chan error), 5)
...
func TestFlushFailed(t *testing.T) {
	ctx, cancel := context.WithCancel(context.TODO())
	b := newBufferSink(ctx, newBlackHoleSink(ctx, map[string]string{}), make(chan error), 5)
	checkpoint, err := b.FlushRowChangedEvents(ctx, 3, 8)
	require.True(t, checkpoint <= 8)
	require.Nil(t, err)
	time.Sleep(200 * time.Millisecond)
