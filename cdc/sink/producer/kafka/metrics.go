...
func (sm *saramaMetricsMonitor) collectProducerMetrics() {
	...
	if histogram, ok := batchSizeMetric.(metrics.Histogram); ok {
		batchSizeGauge.WithLabelValues(sm.changefeedID).Set(histogram.Snapshot().Mean())
	}

	recordSendRateMetric := sm.registry.Get(recordSendRateMetricName)
	if meter, ok := recordSendRateMetric.(metrics.Meter); ok {
		recordSendRateGauge.WithLabelValues(sm.changefeedID).Set(meter.Snapshot().Rate1())
	}
	...
	if histogram, ok := recordPerRequestMetric.(metrics.Histogram); ok {
		recordPerRequestGauge.WithLabelValues(sm.changefeedID).Set(histogram.Snapshot().Mean())
	}
	...
	if histogram, ok := compressionRatioMetric.(metrics.Histogram); ok {
		compressionRatioGauge.WithLabelValues(sm.changefeedID).Set(histogram.Snapshot().Mean())
	}
}
...
func (sm *saramaMetricsMonitor) collectBrokerMetrics() {
	...
	for id := range sm.brokers {
		...
		if meter, ok := incomingByteRateMetric.(metrics.Meter); ok {
			incomingByteRateGauge.WithLabelValues(sm.changefeedID, brokerID).Set(meter.Snapshot().Rate1())
		}
		...
		if meter, ok := outgoingByteRateMetric.(metrics.Meter); ok {
			outgoingByteRateGauge.WithLabelValues(sm.changefeedID, brokerID).Set(meter.Snapshot().Rate1())
		}
		...
		if meter, ok := requestRateMetric.(metrics.Meter); ok {
			requestRateGauge.WithLabelValues(sm.changefeedID, brokerID).Set(meter.Snapshot().Rate1())
		}
		...
		if histogram, ok := requestSizeMetric.(metrics.Histogram); ok {
			requestSizeGauge.WithLabelValues(sm.changefeedID, brokerID).Set(histogram.Snapshot().Mean())
		}
		...
		if histogram, ok := requestLatencyMetric.(metrics.Histogram); ok {
			requestLatencyInMsGauge.WithLabelValues(sm.changefeedID, brokerID).Set(histogram.Snapshot().Mean())
		}
		...
		if counter, ok := requestsInFlightMetric.(metrics.Counter); ok {
			requestsInFlightGauge.WithLabelValues(sm.changefeedID, brokerID).Set(float64(counter.Snapshot().Count()))
		}
		...
		if meter, ok := responseRateMetric.(metrics.Meter); ok {
			responseRateGauge.WithLabelValues(sm.changefeedID, brokerID).Set(meter.Snapshot().Rate1())
		}
...
func (sm *saramaMetricsMonitor) cleanUpProducerMetrics() {
	batchSizeGauge.DeleteLabelValues(sm.changefeedID)
	recordSendRateGauge.DeleteLabelValues(sm.changefeedID)
	recordPerRequestGauge.DeleteLabelValues(sm.changefeedID)
...
func (sm *saramaMetricsMonitor) cleanUpBrokerMetrics() {
	for id := range sm.brokers {
		...
		requestSizeGauge.DeleteLabelValues(sm.changefeedID, brokerID)
		requestLatencyInMsGauge.DeleteLabelValues(sm.changefeedID, brokerID)
