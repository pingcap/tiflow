...
func (t *TableDispatcher) DispatchRowChangedEvent(row *model.RowChangedEvent, partitionNum int32) (int32, string, error) {
	...
	t.hasher.Write([]byte(*row.TableInfo.GetSchemaName()), []byte(*row.TableInfo.GetSchemaName()))
	return int32(t.hasher.Sum32() % uint32(partitionNum)), row.TableInfo.GetSchemaAndTableString(), nil
}
