...
func (w *worker) nonBatchEncodeRun(ctx context.Context) error {
	...
	for {
		...
		case event, ok := <-w.msgChan.Out():
			...
			if err := w.encoderGroup.AddEvents(ctx, event.key.Topic, event.key.Partition,
				event.key.PartitionKey, event.rowEvent); err != nil {
...
func (w *worker) batchEncodeRun(ctx context.Context) (retErr error) {
	...
	for {
		...
		for key, events := range partitionedRows {
			if err := w.encoderGroup.AddEvents(ctx, key.Topic, key.Partition, key.PartitionKey, events...); err != nil {
				return errors.Trace(err)
			}
		}
	}
		...
		case future, ok := <-inputCh:
			...
			for _, message := range future.Messages {
				...
				if err = w.statistics.RecordBatchExecution(func() (int, error) {
					if err := w.producer.AsyncSendMessage(ctx, future.Topic, future.Partition,
						future.WrapperMessagePartitionKey(message)); err != nil {
						return 0, err
					}
