...
func (s *codecInterfaceSuite) TestCreate(c *check.C) {
	...
	msg := NewMQMessage(config.ProtocolOpenProtocol, []byte("key1"), []byte("value1"), rowEvent.CommitTs, model.MqMessageTypeRow, &rowEvent.Table.Schema, &rowEvent.Table.Table)
	...
	c.Assert(msg.Protocol, check.Equals, config.ProtocolOpenProtocol)
