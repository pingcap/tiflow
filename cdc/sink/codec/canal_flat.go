...
func (c *CanalFlatEventBatchEncoder) EncodeCheckpointEvent(ts uint64) (*MQMessage, error) {
	...
	key, _ := json.Marshal(model.MqMessageTypeResolved)
	return newResolvedMQMessage(config.ProtocolCanalJSON, key, value, ts), nil
}
...
func (c *CanalFlatEventBatchEncoder) EncodeDDLEvent(e *model.DDLEvent) (*MQMessage, error) {
	...
	key, _ := json.Marshal(model.MqMessageTypeDDL)
	return newDDLMQMessage(config.ProtocolCanalJSON, key, value, e), nil
}
...
func (c *CanalFlatEventBatchEncoder) Build() []*MQMessage {
	...
	key, _ := json.Marshal(model.MqMessageTypeRow)
	...
	for i, msg := range c.resolvedBuf {
		...
		ret[i] = NewMQMessage(config.ProtocolCanalJSON, key, value, msg.getTikvTs(), model.MqMessageTypeRow, msg.getSchema(), msg.getTable())
	}
