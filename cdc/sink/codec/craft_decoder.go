...
// Events decoder
func decodeRowChangedEvent(keys *craftColumnarKeys, keyIndex int, sizeTables [][]uint64, bits []byte) (*model.RowChangedEvent, error) {
	ev := &model.RowChangedEvent{}
	ev.CommitTs = keys.getTs(keyIndex)
	ev.Table = &model.TableName{
		Schema: keys.getSchema(keyIndex),
		Table:  keys.getTable(keyIndex),
	}
	partition := keys.getPartition(keyIndex)
	if partition >= 0 {
		ev.Table.TableID = partition
		ev.Table.IsPartition = true
	}
	var columnGroup *craftColumnarColumnGroup
	var err error
	columnGroupSizeTable := sizeTables[keyIndex]
	columnGroupIndex := 0
	for len(bits) > 0 {
		columnGroupSize := columnGroupSizeTable[columnGroupIndex]
		columnGroup, err = decodeCraftColumnarColumnGroup(bits[:columnGroupSize])
