...
func (e *craftBatchEncoder) EncodeCheckpointEvent(ts uint64) (*codec.MQMessage, error) {
	return codec.NewResolvedMsg(
...
func (e *craftBatchEncoder) EncodeDDLEvent(ev *model.DDLEvent) (*codec.MQMessage, error) {
	return codec.NewDDLMsg(config.ProtocolCraft,
		nil, NewDDLEventEncoder(e.allocator, ev).Encode(), ev), nil
}

// Build implements the EventBatchEncoder interface
func (e *craftBatchEncoder) Build() []*codec.MQMessage {
	if e.rowChangedBuffer.Size() > 0 {
		// flush buffered data to message buffer
		e.flush()
	}
	ret := e.messageBuf
	e.messageBuf = make([]*codec.MQMessage, 0, 2)
	return ret
}
...
func (e *craftBatchEncoder) flush() {
	...
	mqMessage := codec.NewMsg(config.ProtocolCraft,
...
func newCraftBatchEncoder() codec.EventBatchEncoder {
	...
	return newCraftBatchEncoderWithAllocator(NewSliceAllocator(64))
}
...
func (b *craftBatchEncoderBuilder) Build() codec.EventBatchEncoder {
...
func newCraftBatchEncoderBuilder(config *codec.Config) codec.EncoderBuilder {
...
func newCraftBatchEncoderWithAllocator(allocator *SliceAllocator) codec.EventBatchEncoder {
	return &craftBatchEncoder{
		allocator:        allocator,
		messageBuf:       make([]*codec.MQMessage, 0, 2),
