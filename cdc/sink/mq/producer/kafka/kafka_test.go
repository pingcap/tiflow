...
func TestAdjustConfigTopicExist(t *testing.T) {
	...
	err = adminClient.CreateTopic(nil, topicName, detail, false)
	require.Nil(t, err)
...
func TestAdjustConfigMinInsyncReplicas(t *testing.T) {
	...
	require.Nil(t, err)
	err = adminClient.CreateTopic(nil, topicName, &kafka.TopicDetail{ReplicationFactor: 1}, false)
	...
	err = adminClient.CreateTopic(nil, topicName, &kafka.TopicDetail{
		ReplicationFactor: 3,
		NumPartitions:     3,
	}, false)
	require.Nil(t, err)
