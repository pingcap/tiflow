...
func (m *kafkaTopicManager) getMetadataOfTopics() (map[string]kafka.TopicDetail, error) {
	...
	topicMetaList, err := m.admin.GetTopicsMeta(nil, topicList, true)
	if err != nil {
		log.Warn(
...
func (m *kafkaTopicManager) waitUntilTopicVisible(topicName string) error {
	...
	err := retry.Do(context.Background(), func() error {
		start := time.Now()
		meta, err := m.admin.GetTopicsMeta(nil, topics, false)
		if err != nil {
...
func (m *kafkaTopicManager) listTopics() error {
	start := time.Now()
	topics, err := m.admin.GetAllTopicsMeta(nil)
	if err != nil {
		log.Error(
...
func (m *kafkaTopicManager) createTopic(topicName string) (int32, error) {
	...
	start := time.Now()
	err = m.admin.CreateTopic(nil, topicName, &kafka.TopicDetail{
