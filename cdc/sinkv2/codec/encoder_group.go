...
type EncoderGroup interface {
	...
	// AddEvents add multiple callbackable row changed events to the same encoder
...
func (g *encoderGroup) runEncoder(ctx context.Context, idx int) error {
	...
	for {
		...
		case future := <-inputCh:
			if err := encoder.AppendRowChangedEvents(ctx, future.Topic, future.events); err != nil {
				return err
			}
