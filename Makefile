...
unit_test: check_failpoint_ctl generate_mock generate-msgp-code generate-protobuf
	...
	$(GOTEST) -cover -covermode=atomic -coverprofile="$(TEST_DIR)/cov.unit.out" $(PACKAGE_LIST_WITHOUT_DM_ENGINE) \
...
check-static: tools/bin/golangci-lint
	tools/bin/golangci-lint run --timeout 10m0s --skip-files kv_gen --skip-dirs dm,tests,engine
...
df-master:
	go build -o bin/df-master ./engine/cmd/master
	cp ./bin/df-master ./engine/ansible/roles/common/files/master.bin

df-executor:
	go build -o bin/df-executor ./engine/cmd/executor
	cp ./bin/df-executor ./engine/ansible/roles/common/files/executor.bin

df-master-client:
	go build -o bin/df-master-client ./engine/cmd/master-client

df-demo:
	go build -o bin/df-demoserver ./engine/cmd/demoserver
