...
func TestQueryStatusAPI(t *testing.T) {
	...
	jm.taskManager = NewTaskManager(nil, nil, nil, jm.Logger(), promutil.NewFactory4Test(t.Name()))
...
func TestOperateTask(t *testing.T) {
	jm := &JobMaster{
		taskManager: NewTaskManager(nil, metadata.NewJobStore(kvmock.NewMetaMock(), log.L()), nil, log.L(), promutil.NewFactory4Test(t.Name())),
	}
...
func TestUpdateJobCfg(t *testing.T) {
	...
	jm.taskManager = NewTaskManager(nil, jm.metadata.JobStore(), messageAgent, jm.Logger(), promutil.NewFactory4Test(t.Name()))
