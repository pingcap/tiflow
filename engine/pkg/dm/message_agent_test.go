...
func TestMessagePair(t *testing.T) {
	...
	go func() {
		...
		defer cancel3()
		resp3, err := messagePair.sendRequest(ctx3, "request-topic", "request", mockSender)
	...
	mockSender.On("SendMessage").Return(nil).Once()
	require.NoError(t, messagePair.sendResponse(ctx, "response-topic", 3, resp2, mockSender))
...
func TestMessageAgent(t *testing.T) {
	...
	require.Error(t, messageAgent.SendMessage(context.Background(), "wrong-id", "topic", "msg"), "sender wrong-id not found")
	require.Error(t, messageAgent.SendResponse(context.Background(), "wrong-id", "topic", 1, "resp"), "sender wrong-id not found")
	ret, err := messageAgent.SendRequest(context.Background(), "wrong-id", "topic", "request")
	require.EqualError(t, err, "sender wrong-id not found")
	require.Nil(t, ret)

	mockSender.On("SendMessage").Return(nil).Once()
	require.NoError(t, messageAgent.SendMessage(context.Background(), senderID, "topic", "msg"))
	...
	go func() {
		...
		defer cancel()
		resp2, err := messageAgent.SendRequest(ctx, senderID, "request-topic", "request")
		require.NoError(t, err)
	...
	// send response
	mockSender.On("SendMessage").Return(nil).Once()
	require.NoError(t, messageAgent.SendResponse(context.Background(), senderID, "response-topic", 2, "response"))
	messageAgent.OnMessage(senderID, "response-topic", message{ID: 2, Type: responseTp, Payload: resp})
}
...
func TestMessageHandler(t *testing.T) {
	...
	// mock no handler
	messageAgent := NewMessageAgent(context.Background(), nil, &MockNothing{})
	require.EqualError(t, messageAgent.OnMessage(senderID, "topic", "msg"), "unknow message type for topic topic")
	// register handler
	messageAgent.RegisterHandler("topic", func(interface{}) error { return nil })
	require.NoError(t, messageAgent.OnMessage(senderID, "topic", "msg"))

	// mock no handler
	msg := message{ID: 0, Type: messageTp, Payload: MessageAPIMessage{}}
	require.EqualError(t, messageAgent.OnMessage(senderID, messageAPI, msg), "message handler for topic MessageAPI not found")
	req := message{ID: 1, Type: requestTp, Payload: RequestAPIRequest{}}
	require.EqualError(t, messageAgent.OnMessage(senderID, requestAPI, req), "request handler for topic RequestAPI not found")

	// mock has handler
	mockHandler := &MockHanlder{}
	messageAgent = NewMessageAgent(context.Background(), nil, mockHandler)
	mockSender := &MockSender{}
	messageAgent.UpdateSender(senderID, mockSender)
	mockSender.On("SendMessage").Return(nil).Once()
	// handle message
	...
	require.NoError(t, messageAgent.OnMessage(senderID, messageAPI, msg))
	mockHandler.On(messageAPI).Return(errors.New("error")).Once()
	require.EqualError(t, messageAgent.OnMessage(senderID, messageAPI, msg), "error")
	// handle request
	req = message{ID: 1, Type: requestTp, Payload: RequestAPIRequest{}}
	mockHandler.On(requestAPI).Return(RequestAPIResponse{}, nil).Once()
	require.NoError(t, messageAgent.OnMessage(senderID, requestAPI, req))
	mockHandler.On(requestAPI).Return(RequestAPIResponse{}, errors.New("error")).Once()
	require.EqualError(t, messageAgent.OnMessage(senderID, requestAPI, req), "error")
	// handle response
	res := message{ID: 1, Type: responseTp, Payload: RequestAPIResponse{}}
	require.EqualError(t, messageAgent.OnMessage(senderID, requestAPI, res), "request 1 not found")
}
