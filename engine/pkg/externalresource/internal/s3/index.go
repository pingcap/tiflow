...
type indexManager interface {
	...
	) (map[resModel.ResourceName]struct{}, error)

	GetPersistedFileMap() map[resModel.WorkerID]*indexFile

	Close()
}

type indexFile struct {
	PersistedFileSet map[resModel.ResourceName]struct{} `json:"persisted_file_set"`
}

func newIndexFile() *indexFile {
	return &indexFile{
		PersistedFileSet: map[resModel.ResourceName]struct{}{},
	}
}
...
func (f *indexFile) GetPersistedFileSet() map[resModel.ResourceName]struct{} {
...
type indexManagerImpl struct {
	...
	indexMu     sync.Mutex
	indexFiles  map[resModel.WorkerID]*indexFile
	updateCount int
...
func newIndexManager(
	...
	ret := &indexManagerImpl{
		...
		indexFiles: make(map[resModel.WorkerID]*indexFile),
...
func (m *indexManagerImpl) GetPersistedFileMap() map[resModel.WorkerID]*indexFile {
	return m.indexFiles
}
...
func (m *indexManagerImpl) triggerFlush(ctx context.Context, workerID resModel.WorkerID) error {
