...
func testInitialize(t *testing.T) {
	...
	mock.ExpectExec(regexp.QuoteMeta("CREATE TABLE `master_meta_kv_data` (`seq_id` bigint unsigned AUTO_INCREMENT,`created_at` datetime(3) NULL," +
		...
		"`config` blob,`error_message` text,`ext_bytes` blob,`deleted` datetime(3) NULL,PRIMARY KEY (`seq_id`),INDEX `idx_mst` (`project_id`,`status`),UNIQUE INDEX `uidx_mid` (`id`))")).WillReturnResult(sqlmock.NewResult(1, 1))
	...
	mock.ExpectExec(regexp.QuoteMeta("CREATE TABLE `worker_statuses` (`seq_id` bigint unsigned AUTO_INCREMENT,`created_at` datetime(3) NULL," +
		...
		"`epoch` bigint not null,`errmsg` text,`ext_bytes` blob,PRIMARY KEY (`seq_id`),UNIQUE INDEX `uidx_wid` (`job_id`,`id`),INDEX `idx_wst` (`job_id`,`status`))")).WillReturnResult(sqlmock.NewResult(1, 1))
