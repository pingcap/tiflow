...
func testInitialize(t *testing.T) {
	...
	mock.ExpectExec(regexp.QuoteMeta("CREATE TABLE `master_meta` (`seq_id` bigint unsigned AUTO_INCREMENT,`created_at` datetime(3) NULL," +
		...
		"`config` blob,`extend_message` text,`deleted` datetime(3) NULL,PRIMARY KEY (`seq_id`),INDEX `idx_mst` (`project_id`,`state`),UNIQUE INDEX `uidx_mid` (`id`))")).WillReturnResult(sqlmock.NewResult(1, 1))
