...
func NewPrefixKV(kv internal.KVExt, prefix string) metaclient.KV {
...
func (kv *kvPrefix) Get(ctx context.Context, key string, opts ...metaclient.OpOption) (*metaclient.GetResponse, metaclient.Error) {
	...
	r, err := kv.KVExt.Do(ctx, kv.prefixOp(metaclient.OpGet(key, opts...)))
	if err != nil {
		return nil, err
	}
	get := r.Get()
...
func (kv *kvPrefix) Delete(ctx context.Context, key string, opts ...metaclient.OpOption) (*metaclient.DeleteResponse, metaclient.Error) {
	...
	r, err := kv.KVExt.Do(ctx, kv.prefixOp(metaclient.OpDelete(key, opts...)))
	if err != nil {
		return nil, err
	}
	del := r.Del()
