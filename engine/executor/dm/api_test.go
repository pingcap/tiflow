...
func TestQueryStatusAPI(t *testing.T) {
	var (
		...
		dumpStatusResp     = &dmproto.QueryStatusResponse{
			Unit:         frameModel.WorkerDMDump,
			Stage:        metadata.StageRunning,
			Status:       []byte(dumpStatusBytes),
			IoTotalBytes: 0,
		}
		loadStatusResp = &dmproto.QueryStatusResponse{
			Unit:         frameModel.WorkerDMLoad,
			Stage:        metadata.StageFinished,
			Result:       &dmproto.ProcessResult{IsCanceled: false},
			Status:       []byte(loadStatusBytes),
			IoTotalBytes: 0,
		}
		syncStatusResp = &dmproto.QueryStatusResponse{
			Unit:         frameModel.WorkerDMSync,
			Stage:        metadata.StagePaused,
			Result:       &dmproto.ProcessResult{Errors: []*dmproto.ProcessError{processError}},
	...
	require.Equal(t, dmWorker.QueryStatus(context.Background(), &dmproto.QueryStatusRequest{Task: "wrong-task-id"}).ErrorMsg,
	...
	resp := dmWorker.QueryStatus(context.Background(), &dmproto.QueryStatusRequest{Task: "task-id"})
...
func TestStopWorker(t *testing.T) {
	...
	require.EqualError(t, dmWorker.StopWorker(context.Background(), &dmproto.StopWorkerMessage{Task: "wrong-task-id"}), "task id mismatch, get wrong-task-id, actually task-id")
	err = dmWorker.StopWorker(context.Background(), &dmproto.StopWorkerMessage{Task: "task-id"})
	require.NoError(t, err)
...
func TestOperateTask(t *testing.T) {
	...
	require.EqualError(t, dmWorker.OperateTask(context.Background(), &dmproto.OperateTaskMessage{Task: "wrong-task-id"}), "task id mismatch, get wrong-task-id, actually task-id")
	mockUnitHolder.On("Pause").Return(nil).Once()
	require.Nil(t, dmWorker.OperateTask(context.Background(), &dmproto.OperateTaskMessage{Task: "task-id", Op: dmproto.Pause}))
	mockUnitHolder.On("Resume").Return(nil).Once()
	require.Nil(t, dmWorker.OperateTask(context.Background(), &dmproto.OperateTaskMessage{Task: "task-id", Op: dmproto.Resume}))
	require.EqualError(t, dmWorker.OperateTask(context.Background(), &dmproto.OperateTaskMessage{Task: "task-id", Op: dmproto.Update}),
		fmt.Sprintf("unsupported op type %d for task %s", dmproto.Update, "task-id"))
}
...
func TestBinlog(t *testing.T) {
	...
	require.Equal(t, "error", dmWorker.BinlogTask(context.Background(), &dmproto.BinlogTaskRequest{Task: "task-id"}).ErrorMsg)
	mockUnitHolder.On("Binlog").Return("msg", nil).Once()
	require.Equal(t, "msg", dmWorker.BinlogTask(context.Background(), &dmproto.BinlogTaskRequest{Task: "task-id"}).Msg)
}
...
func TestBinlogSchema(t *testing.T) {
	...
	require.Equal(t, "task id mismatch, get wrong-task-id, actually task-id", dmWorker.BinlogSchemaTask(context.Background(), &dmproto.BinlogSchemaTaskRequest{Source: "wrong-task-id"}).ErrorMsg)
	mockUnitHolder.On("BinlogSchema").Return("", errors.New("error")).Once()
	require.Equal(t, "error", dmWorker.BinlogSchemaTask(context.Background(), &dmproto.BinlogSchemaTaskRequest{Source: "task-id"}).ErrorMsg)
	mockUnitHolder.On("BinlogSchema").Return("msg", nil).Once()
	require.Equal(t, "msg", dmWorker.BinlogSchemaTask(context.Background(), &dmproto.BinlogSchemaTaskRequest{Source: "task-id"}).Msg)
}
