...
function run() {
    ...
    run_sql "CREATE table kafka_sink_error_resume.t1(id int primary key clustered auto_increment, val int);" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
    run_sql "CREATE table kafka_sink_error_resume.t2(id int primary key clustered auto_increment, val int);" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
