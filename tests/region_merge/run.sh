...
function split_and_random_merge() {
    ...
    pd-ctl --pd=$pd_addr scheduler remove random-merge-scheduler || true
    ...
    pd-ctl --pd=$pd_addr scheduler add random-merge-scheduler
...
function run() {
    ...
    pd-ctl --pd=$pd_addr config set split-merge-interval 1s
    pd-ctl --pd=$pd_addr config set merge-schedule-limit 128
