...
function check_resolved_ts() {
	...
	check_tso=$2
	redo_dir=$3
	rts=$(cdc redo meta --storage="nfs:///tmp/tidb_cdc_test/consistent_replicate_schedulerv3/nfs/redo" --tmp-dir="$redo_dir" | grep -oE "resolved-ts:[0-9]+" | awk -F: '{print $2}')
...
function run() {
	...
	run_sql "CREATE DATABASE consistent_replicate_schedulerv3;" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
	go-ycsb load mysql -P $CUR/conf/workload -p mysql.host=${UP_TIDB_HOST} -p mysql.port=${UP_TIDB_PORT} -p mysql.user=root -p mysql.db=consistent_replicate_schedulerv3
	run_sql "CREATE table consistent_replicate_schedulerv3.check1(id int primary key);" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
	check_table_exists "consistent_replicate_schedulerv3.USERTABLE" ${DOWN_TIDB_HOST} ${DOWN_TIDB_PORT}
	check_table_exists "consistent_replicate_schedulerv3.check1" ${DOWN_TIDB_HOST} ${DOWN_TIDB_PORT}
	...
	run_cdc_server --workdir $WORK_DIR --binary $CDC_BINARY --config $CUR/conf/server.toml
	run_sql "create table consistent_replicate_schedulerv3.USERTABLE2 like consistent_replicate_schedulerv3.USERTABLE" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
