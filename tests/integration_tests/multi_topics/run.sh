...
function run() {
	...
	SINK_URI="kafka://127.0.0.1:9093/$DEFAULT_TOPIC_NAME?protocol=canal-json&enable-tidb-extension=true&kafka-version=${KAFKA_VERSION}"
	...
	for i in $(seq 1 3); do
		run_kafka_consumer $WORK_DIR "kafka://127.0.0.1:9093/test_table${i}?protocol=canal-json&version=${KAFKA_VERSION}&enable-tidb-extension=true" "" ${i}
	done
	...
	sleep 30
	run_kafka_consumer $WORK_DIR "kafka://127.0.0.1:9093/test_table10?protocol=canal-json&version=${KAFKA_VERSION}&enable-tidb-extension=true" "" 1
	run_kafka_consumer $WORK_DIR "kafka://127.0.0.1:9093/test_table20?protocol=canal-json&version=${KAFKA_VERSION}&enable-tidb-extension=true" "" 2
	run_kafka_consumer $WORK_DIR "kafka://127.0.0.1:9093/test_finish?protocol=canal-json&version=${KAFKA_VERSION}&enable-tidb-extension=true" "" "finish"
