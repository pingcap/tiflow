...
function run() {
	...
	run_sql "CREATE table testSync.fake_table_avoid_sync_diff_no_table_fatal(id int primary key, val int);"
	run_sql "CREATE table testSync.fake_table_avoid_sync_diff_no_table_fatal(id int primary key, val int);" ${DOWN_TIDB_HOST} ${DOWN_TIDB_PORT}
