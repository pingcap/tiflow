...
stop() {
	...
	echo $(mysql -h${DOWN_TIDB_HOST} -P${DOWN_TIDB_PORT_1} -uroot -e "show databases;")
	stop_tidb_cluster
}
...
function run() {
	...
	check_table_exists "partition_table2.t2" ${DOWN_TIDB_HOST} ${DOWN_TIDB_PORT_1} 120
	sleep 5
	...
	# check_table_exists "partition_table.finish_mark" ${DOWN_TIDB_HOST} ${DOWN_TIDB_PORT_1} 120
