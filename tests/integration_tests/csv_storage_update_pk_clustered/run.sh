...
function run() {
	...
	SINK_URI3="file://$WORK_DIR/storage_test/changefeed3?flush-interval=5s"
	run_cdc_cli changefeed create --sink-uri="$SINK_URI3" --config=$CUR/conf/changefeed3.toml -c "changefeed3"
	SINK_URI4="file://$WORK_DIR/storage_test/changefeed4?flush-interval=5s"
	run_cdc_cli changefeed create --sink-uri="$SINK_URI4" --config=$CUR/conf/changefeed4.toml -c "changefeed4"
	...
	run_storage_consumer $WORK_DIR $SINK_URI3 $CUR/conf/changefeed3.toml ""
	sleep 8
	check_sync_diff $WORK_DIR $CUR/conf/diff_config.toml 100

	run_storage_consumer $WORK_DIR $SINK_URI4 $CUR/conf/changefeed4.toml ""
	sleep 8
	check_sync_diff $WORK_DIR $CUR/conf/diff_config.toml 100
}
