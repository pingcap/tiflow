...
function prepare() {
	...
	# Prepare a table
	run_sql "CREATE TABLE round VALUES(round INT, val INT, success BOOLEAN);" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
	check_table_exists test.round ${DOWN_TIDB_HOST} ${DOWN_TIDB_PORT_1} 60
}
...
function run() {
	...
	run_sql "INSERT INTO round VALUES(1, 1, true);" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
	...
	sleep 5
	kill $pid2
	run_sql "INSERT INTO round VALUES(2, 1, true);" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
	run_sql "COMMIT;" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
	...
	# tidb 1 should works
	run_sql "BEGIN;" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
	run_sql "INSERT INTO round VALUES(3, 1, true);" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
	sleep 5
	...
	# tidb 2 should works
	run_sql "BEGIN;" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
	run_sql "INSERT INTO round VALUES(4, 1, true);" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
	sleep 5
	start_downstream_tidb_instances --db 1 --out_dir $WORK_DIR --suffix 2
	mapfile -t down_tidb_pids < "$WORK_DIR/downstream_tidb_instances_pids.log"
	pid2 = ${down_tidb_pids[0]}
	kill $pid1
	run_sql "INSERT INTO round VALUES(4, 2, true);" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
	run_sql "COMMIT;" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
	check_sync_diff $WORK_DIR $CUR/diff_config_2.toml
	...
	# tidb 3 should works
	run_sql "BEGIN;" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
	run_sql "INSERT INTO round VALUES(5, 1, true);" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
	run_sql "COMMIT;" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
	sleep 5
	start_downstream_tidb_instances --db 1 --out_dir $WORK_DIR --suffix 3
	kill $pid2
	run_sql "INSERT INTO round VALUES(5, 2, true);" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
	run_sql "COMMIT;" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
	check_sync_diff $WORK_DIR $CUR/diff_config_3.toml
}
