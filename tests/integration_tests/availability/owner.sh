...
function test_kill_owner() {
	...
	capture_id=$($CDC_BINARY cli capture list --server 'http://127.0.0.1:8301' --disable-version-check 2>&1 | awk -F '"' '/id/{print $4}' | grep -v "$owner_id")
...
function test_hang_up_owner() {
	...
	ensure $MAX_RETRIES "$CDC_BINARY cli capture list --server 'http://127.0.0.1:8301'  --disable-version-check 2>&1 | grep -v \"$owner_id\" | grep id"
...
function test_owner_cleanup_stale_tasks() {
	...
	ensure $MAX_RETRIES "$CDC_BINARY cli capture list --server 'http://127.0.0.1:8302' --disable-version-check 2>&1 | grep '\"is-owner\": true'"
