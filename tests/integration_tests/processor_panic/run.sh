...
function prepare() {
	...
	#               --failpoint 'github.com/pingcap/tiflow/cdc/ProcessorSyncResolvedPreEmit=return(true)' # old processor

	run_cdc_server --workdir $WORK_DIR --binary $CDC_BINARY --logsuffix 1 --addr 127.0.0.1:8300 --restart true \
		--failpoint 'github.com/pingcap/tiflow/cdc/processor/pipeline/ProcessorSyncResolvedPreEmit=return(true)' # new processor
