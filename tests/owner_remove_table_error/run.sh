...
function run() {
    ...
    run_sql "CREATE table owner_remove_table_error.t1(id int primary key clustered auto_increment, val int);" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
    ...
    run_sql "CREATE table owner_remove_table_error.t2(id int primary key clustered auto_increment, val int);" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
    run_sql "INSERT INTO owner_remove_table_error.t2 VALUES (),(),();" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
    run_sql "CREATE table owner_remove_table_error.finished_mark(id int primary key clustered auto_increment, val int);" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
