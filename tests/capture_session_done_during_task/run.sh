...
function run() {
    ...
    run_sql "CREATE table capture_session_done_during_task.t (id int primary key auto clustered _increment, a int)" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
    run_sql "CREATE DATABASE capture_session_done_during_task;" ${DOWN_TIDB_HOST} ${DOWN_TIDB_PORT}
    run_sql "CREATE table capture_session_done_during_task.t (id int primary key auto clustered _increment, a int)" ${DOWN_TIDB_HOST} ${DOWN_TIDB_PORT}
