...
function run() {
    ...
    run_sql "CREATE table unified_sort.check1(id int primary key clustered );" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
    ...
    check_sync_diff $WORK_DIR $CUR/conf/diff_config.toml
    run_sql "CREATE table unified_sort.check2(id int primary key clustered );" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
    check_table_exists "unified_sort.check2" ${DOWN_TIDB_HOST} ${DOWN_TIDB_PORT} 90
    check_sync_diff $WORK_DIR $CUR/conf/diff_config.toml

    go-ycsb load mysql -P $CUR/conf/workload -p mysql.host=${UP_TIDB_HOST} -p mysql.port=${UP_TIDB_PORT} -p mysql.user=root -p mysql.db=unified_sort
    run_sql "CREATE table unified_sort.check3(id int primary key clustered );" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
    ...
    run_sql "create table unified_sort.check4(id int primary key clustered );" ${UP_TIDB_HOST} ${UP_TIDB_PORT}
