...
function prepare() {
    ...
    run_cdc_server --workdir $WORK_DIR --binary $CDC_BINARY --addr "127.0.0.1:8300" --logsuffix 1 --data-dir "${WORK_DIR}"/cdc_data1
    ...
    check_table_exists "test.simple2" ${DOWN_TIDB_HOST} ${DOWN_TIDB_PORT}
    check_table_exists "test.simple3" ${DOWN_TIDB_HOST} ${DOWN_TIDB_PORT}
    check_table_exists "test.simple4" ${DOWN_TIDB_HOST} ${DOWN_TIDB_PORT}

    # starts the second server instance. It should fail, and bring down the changefeed, due to sort-dir conflict
